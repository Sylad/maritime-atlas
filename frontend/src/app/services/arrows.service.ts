...
export interface ArrowsManifest {
  ...
  arpege?: string[];   // ARPEGE 10m wind (Météo-France) — optionnel.
  arome?: string[];    // AROME 10m wind (Météo-France, FR métropole HD).
  patterns: { wind: string; wave: string; arpege?: string; arome?: string };
  updated_at: string;
}

// 'wind' = GFS (NOAA, fichiers wind_arrows_*.geojson)
// 'arpege' = ARPEGE (Météo-France Europe, fichiers arpege_wind_arrows_*.geojson)
// 'arome' = AROME (Météo-France FR HD, fichiers arome_wind_arrows_*.geojson)
// 'wave' = WW3 (NOAA, wave_arrows_*.geojson)
export type ArrowsKind = 'wind' | 'wave' | 'arpege' | 'arome';
...
export class ArrowsService {
  ...
  /** GeoJSON pour un kind (wind|wave|arpege|arome) à un ts donné. ARPEGE
   *  et AROME utilisent un préfixe distinct (volume partagé avec GFS,
   *  disambiguation côté nom de fichier). */
  fetchArrows(kind: ArrowsKind, ts: string): Promise<ArrowsFeatureCollection> {
    const url = kind === 'arpege' ? `/wind-arrows/arpege_wind_arrows_${ts}.geojson`
              : kind === 'arome'  ? `/wind-arrows/arome_wind_arrows_${ts}.geojson`
