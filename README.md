...
| GS custom plugins | **2 plugins Java maison** baked dans `WEB-INF/lib/` : (1) **`maritime-gwc-init`** — Spring `ApplicationListener<ContextRefreshedEvent>` qui crée idempotemment le BlobStore S3 + assigne 16 layers GWC à chaque boot. Config GS reproductible 100% gitops (vs ancien REST PUT non-persistant). (2) **`maritime-cascade-time-forward`** (G65, 2026-05-27) — fix le bug GeoTools `gt-wms` qui ne forwarde JAMAIS `&TIME=...` upstream sur cascade WMS. Remplace `ResourcePool.wmsCache` par `WrappingWmsCache` via réflexion → chaque `WebMapServer.httpClient` auto-wrappé par un décorateur qui inject TIME (depuis un ThreadLocal alimenté par DispatcherCallback). Validé prod : HRV/RSS/MTG/DWD/KNMI animation cascade fonctionnelle. |
...
| Frontend | Angular 19 + MapLibre 5.24 (carte unique `/`, projection sphère + WebGL particles) + nginx alpine. OpenLayers 10 conservé en dépendance (preview zone `/palettes`) mais la carte OL legacy a été supprimée (G67, 2026-05-28). |
...
| **Globe 3D** (2026-05-22, Phase 5a+5b) | Route `/globe` MapLibre 5.24 projection sphère + WebGL custom layer wind particles (port `webgl-wind` Mapbox ISC). SST WMS + 12 satellites (NASA GIBS + EUMETSAT cascade + radar DWD/KNMI) + 3 vector layers (lightning + alerts + vessels AIS cluster). Cohabitait avec `/legacy-map` OL10 (supprimé G67 2026-05-28). |
| **G67 — suppression carte OL legacy** (2026-05-28) | Le globe MapLibre ayant atteint la parité features, l'ancienne carte 2D OpenLayers (`map.component.ts`, 8049 lignes, route `/legacy-map`) est supprimée : 2 implémentations à maintenir = confusion en dev. `/` sert le globe, `/legacy-map` et tout chemin inconnu → wildcard redirect `/`. Dep `ol@10` conservée (preview zone `/palettes`). |
...
| **Cascade TIME forward — G65** (2026-05-27, 11 commits) | Fix le bug GS core : `gt-wms` n'a 0 mention de TIME → cascade WMS ne forwarde JAMAIS `&TIME=...` à l'upstream EUMETSAT/DWD/KNMI → animation HRV/RSS/MTG/radars affichait toujours la même image. Plugin Java `maritime-cascade-time-forward` (5 classes, ~450 LOC) : `WrappingWmsCache` remplace `ResourcePool.wmsCache` via réflexion → auto-wrap chaque `WebMapServer.httpClient` sur put() → `CascadeTimeDispatcherCallback.operationDispatched()` stash TIME dans ThreadLocal → `CascadeTimeForwardingHTTPClient.get(URL)` réécrit `&TIME=...` dans URL upstream. Validation prod : HRV 4 TIMEs diurnes → 4 md5 distincts (avant 1 seul). |
